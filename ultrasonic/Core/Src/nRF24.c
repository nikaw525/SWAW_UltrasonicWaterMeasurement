...
void nRF24_WriteRegisters(uint8_t reg, uint8_t* val, uint8_t len)
{
...
void nRF24_send_data_by_radio(Message_T msg)
{
	static uint8_t idx = 0;

	if(idx%2)
	{
		nRF24_WriteTXPayload((uint8_t *)&msg.msg_dist);
	}
	else
	{
		nRF24_WriteTXPayload((uint8_t *)&msg.msg_vbat);
	}
